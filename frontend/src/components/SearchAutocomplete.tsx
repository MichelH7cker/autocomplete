...
export default function SearchAutocomplete() {
  ...
  // debounce
  useEffect(() => {
    // termo curto, esconde a lista e não faz nada
    if (searchTerm.length < 4) {
      setListVisible(false);
      return;
    }
    // busca só acontece se o usuário parar de digitar por 300ms
  ...
  return (
    <div className="w-full">
      ...
      <div className="flex rounded-md shadow-sm relative" ref={wrapperRef}>
        <input
          type="text"
          value={searchTerm}
          onChange={(e) => setSearchTerm(e.target.value)}
          onFocus={() => { if (suggestions.length > 0) setListVisible(true); }}
          placeholder="Digite 4 ou mais caracteres..."
        ...
        {isListVisible && (
          <div className="absolute top-full left-0 right-0 z-10 w-full mt-2 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-600 rounded-lg shadow-lg">
            {loading ? (
              ...
              // exibir 10 com scroll para o resto
              <ul className="max-h-[27.5rem] overflow-y-auto" style={{ scrollbarWidth: "thin" }}>
                {suggestions.slice(0, 10).map((suggestion: { text: string }, index: number) => (
                  <li
                    key={index}
